...
#define PWR_UP_SEC 16
  ...
  void wait_for_32K() __attribute__ ((noinline)) {
    ...
    /*
     * TA1 -> XT1 32768   (just for fun and to compare against TA0 (1uis ticker)
     * TA0 -> SMCLK/1 (should be 1uis ticker)
     */
    TA1CTL = TACLR;			// also zeros out control bits
    TA0CTL = TACLR;
    TA1CTL = TASSEL__ACLK  | MC__CONTINOUS;	//  ACLK/1, continuous
    TA0CTL = TASSEL__SMCLK | MC__CONTINOUS;	// SMCLK/1, continuous
  ...
  command error_t Init.init () {
     ...
     */
    TA1CTL = TASSEL__ACLK | TACLR | MC__CONTINOUS | TAIE;
    TA1R = 0;

    /*
     * TA0 clocked off SMCLK off DCO, /8, 1us tick
     */
    TA0CTL = TASSEL__SMCLK | ID__8 | TACLR | MC__CONTINOUS | TAIE;
    TA0R = 0;
